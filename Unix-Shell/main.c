...
/*This function is responsible for interperting the all_words array into an array of commands
/ rather than taking each word at a time we decided it would be easier to proccess the array
/ of words into an array of commands. This function also keeps track of all redirect symbols
/ as well as file namespace
/ Param: all_words: ws dilemetated array of words
/ Param: size: the number of entries in the all_words array
/ Param: all_redirects: an int array storing the symbols we used to identify redirection
/ Param: list_to_populate: this is the array of commands that is being populated
/ Param: files: this is an array storing all filenames encountered
/ Returns: an int storing the number of entries in the command array
*/
  ...
  while( fgets(line, MAX_LINE_CHARS, stdin) ) 
    {
      ...
      int p = 1; // used to determine which pair to look at
      ...
      for(int i =0; i < num_cmds; i++)
	{
      ...
      for (int j = 0; j < num_cmds; j++) //reap all children
